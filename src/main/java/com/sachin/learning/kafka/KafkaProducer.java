...
public class KafkaProducer implements Callable<Void> {
...
//			props.put("partitioner.class", "com.sachin.learning.kafka.KafkaCustomPartitioner");
    ...
    public Void call() throws Exception {
        ...
        for(int i=0;i<N_ITR;i++){
            String key = getMessagePrevixChar();
            String msg = key + "- ("+producerName+") Test event-" + new Date();
            if(i == N_ITR - 1){
                msg = key + "- Done!!..pls stop the program.";
            }
            System.out.println("\n\nSending msg to consumer(s) - "+msg);
            this.send(new KeyedMessage<String, String>(
                    topic,
                    key,
                    msg));
            Thread.sleep(produceMsgTimeDelay);
        }
        return null;
    }

    private String getMessagePrevixChar(){
